...
const CELL_HIGHLIGHT = '#66B2FF';
const ELECTRON_COLOR = '#0077B6';
const FONT_COLOR = '#CC3333';
...
class Cell {
  ...
  scheduleUpdate(
  t1 = CELL_REPAINT_INTERVAL[0]/2,
  t2 = CELL_REPAINT_INTERVAL[1]/1.5)
  {
...
function createNavbar() {
    ...
    navbar.style.backgroundColor = 'rgba(0,0,0,0)'; // Semi-transparent background
    navbar.style.overflow = 'hidden';
    navbar.style.position = 'fixed';
    navbar.style.top = '0';
    navbar.style.width = '100%';
    navbar.style.fontFamily = 'CustomFont, Arial'; // Use custom font with fallback to Arial
    navbar.style.padding = '1%';
    navbar.style.fontSize = '24px'; // Increase text size
    navbar.style.boxShadow = '0px 4px 8px rgba(0, 0, 0, 0.3)'; // Add a subtle shadow
    ...
    // Create an unordered list (ul) to hold the navigation items
    ...
    // Create navigation items (list items - li) and links (a)
    const navItems = ['Home', 'Machine Learning', 'Mobile Apps','Miscellaneous Projects'];
    const links = ['index.html', 'projects.html', 'about.html','Misc.html' ];
    ...
    navItems.forEach((itemText, index) => {
      ...
      link.href = links[index]; // Set link URLs based on the provided array
      link.textContent = itemText;
      link.style.textDecoration = 'none';
      link.style.color = 'white';
      link.style.position = 'relative'; // Required for the underline effect
      ...
      link.addEventListener('mouseenter', () => {
        link.style.color = '#FF6B6B'; // Change text color on hover
        const underline = document.createElement('div');
        underline.classList.add('underline');
        link.appendChild(underline);
      });
  
      // Remove the underline and reset styles on mouse leave
      ...
      link.addEventListener('click', (event) => {
        ...
        setTimeout(() => {
          ...
          // Navigate to the link URL after the animation
...
// Add some CSS to style the underline effect and click animation
...
// Call the function to create the navbar with the provided links and hover effect
var values = ["Type Here", "#explode", "#galaxy","#ring","#queue","#countdown","#clear"];
var currentIndex = 0;

function changeValue() {
  if (currentIndex < values.length) {
    var inputElement = document.getElementById("input");
    inputElement.style.transition = "color 1s"; // Add a transition effect to the text color
    ...
    setTimeout(function() {
      inputElement.placeholder = values[currentIndex];
      inputElement.style.color = ""; // Reset text color
      currentIndex++; // Move to the next value in the list
    }, 500); // Wait for the transition duration (0.5 seconds) before changing the value
  } else {
    currentIndex = 0;
    var inputElement = document.getElementById("input");
    inputElement.placeholder = values[currentIndex];
